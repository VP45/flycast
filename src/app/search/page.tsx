...
import { FlightLargeType } from "../../../types/Flight";
type Props = {};
...
const ResultPage = (props: Props) => {
  ...
  const [flights, setFlights] = useState<FlightLargeType>();
  ...
  const fetchFlights = async (SrcAirport: Airport | undefined, DstAirport: Airport | undefined, classType : String, date:DateValueType, adults:Number, children:Number ) => {
    fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/flight`, {
      ...
      .then((res) => {
        ...
        //   alert("No flights available for the selected date");
        //   router.push("/");
        // }
        // if(res.data.length > 15){
        //   res.results = res.results.slice(0,15);
        // }
        console.log("flights", JSON.parse(res));
        setFlights(JSON.parse(res));
        localStorage.setItem("flights", JSON.stringify(res.results));
      })
      .catch((err) => console.log(err));
  }
  ...
  useEffect(() => {
    ...
    fetchFlights(DstAirport, SrcAirport, classType, date, adults, childrenn)
  ...
  // useEffect(() => {
  //   if (flights?.data?.length! > 0) {
  //     predictFlightPrice(flights[0].srcAirport, flights[0].dstAirport, flights[0].classType);
  //   }
  // }, [flights]);
    ...
    <div className="w-full sm:w-[98%] md:w-[90%] flex flex-col items-center mx-auto">
      ...
      <div className="mt-4 mb-4 w-full flex flex-col items-center space-y-10">
        ...
        <div className="w-full max-w-6xl text-gray-900 dark:text-white flex flex-col items-center">
          <div className={`pt-12 w-full flex flex-col md:flex-row items-center border-b-4 border-dashed ${test ? "border-[#04ff30]" : "border-red-600"} bg-gray-100 dark:bg-gray-900`}>
            ...
            <div className="md:w-[65%] md:p-16 flex flex-col items-center justify-center">
              <h1 className=" text-2xl sm:text-3xl md:text-4xl font-bold text-center leading-[1.2]">Price{" "}
                <span className={test ? "text-[#04ff30]" : "text-red-600"}>{test ? "Drop" : "Rise" }</span> Chance<br/>
                Oh{" "}
                <span className={test ? "text-[#04ff30]" : "text-red-600"}>{test ? "Yes!" : "No!"}</span>{" "}
                The price could {test? "reduce" : "increase"} by{" "}
                <span className={test ? "text-[#04ff30]" : "text-red-600"}>20%*</span>
              </h1>
              <p>
                • There is a 73% chance of a {test ? "drop" : "rise"} in price
              </p>
            </div>
          </div>
